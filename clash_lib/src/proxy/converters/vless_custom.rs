...
use crate::{
    proxy::{
        vless_custom::{Handler, HandlerOptions},
        HandlerCommonOptions,
    },
    Error,
};
use crate::config::vless_custom::OutboundVlessCustom;
use crate::proxy::transport::{GrpcClient, H2Client, TlsClient, WsClient};

impl TryFrom<OutboundVlessCustom> for Handler {
    type Error = crate::Error;

    fn try_from(value: OutboundVlessCustom) -> Result<Self, Self::Error> {
        (&value).try_into()
    }
}
...
impl TryFrom<&OutboundVlessCustom> for Handler {
    ...
    fn try_from(s: &OutboundVlessCustom) -> Result<Self, Self::Error> {
