...
impl<T: ReadExactBase> ReadExt for T {
    ...
    fn poll_read_once(
        ...
        let (raw, read_buf, read_pos) = self.decompose();
        let mut buffer_vev = Vec::with_capacity(size);
        unsafe {
            buffer_vev.set_len(size);
        }
        let mut buffer = ReadBuf::new(&mut buffer_vev);
        ready!(Pin::new(&mut *raw).poll_read(cx, &mut buffer))?;
        ...
        read_buf.extend_from_slice(buffer.filled());
        *read_pos += buffer.filled().len();
        Poll::Ready(Ok(()))
    }
}
